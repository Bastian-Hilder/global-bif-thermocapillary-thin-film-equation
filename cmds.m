%% main command file for the bifurcation analysis of a thermocapillary thin-film equation
% 
% The command file performs a numerical continuation of the second order
% equation
%   0 = h_xx - (g(1+h)-(3/2)*M*(log((1+h)/(2+h))+1/(2+h))+K(0)+lambda
...
% boundary conditions on a domain (-lx,lx).
...
p = [];
lx = 5*pi;          % set domain size (-lx,lx)
nx = 1000;          % number of discretisation points in domain
...
p = swibra('init','bpt2','1D2'); p = cont(p,40); % branch at M^*(k_0)
p = swibra('init','bpt4','1D4'); p = cont(p,45); % branch at M^*(2k_0)
p = swibra('init','bpt6','1D6'); p = cont(p,55); % branch at M^*(3k_0)

%% c6: plot bifurcation diagram
hold on;
plotbra('init','cl','k');
plotbra('1D2','cl','b','lab',[5,40]);
plotbra('1D4','cl','b','ms',0);
plotbra('1D6','cl','b','ms',0);
hold off;
